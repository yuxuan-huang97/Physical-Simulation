...
// Modified by Yuxuan Huang into a simulated cloth with drag force

String projectTitle = "Simulated Cloth with drag";
...
void update(float dt){
  ...
  PVector acc_drag[] = new PVector[numofthread * numofbead];
  for (int i = 0; i < numofthread * numofbead; i++) acc_drag[i] = new PVector();
  float deltax;
  float deltay;
  float deltaz;
  float len;
  for (int t = 0; t < 25; t++) {
    ...
    PVector va1, va2; // average velocity
    PVector fd1, fd2; // drag force
    PVector a1, a2; // drag acceleration
    float cd = 0.01;
    for (int i = 0; i < numofthread-1; i++) { // each thread
      for (int j = 0; j < numofbead; j++) { // each bead
        va1 = new PVector(c.t[i].b[j].vel.x, c.t[i].b[j].vel.y, c.t[i].b[j].vel.z);
        va2 = new PVector(c.t[i].b[j].vel.x, c.t[i].b[j].vel.y, c.t[i].b[j].vel.z);
        if (j == 0) {
          r1 = new PVector(anchorX + i*disT - c.t[i].b[j].pos.x, anchorY-c.t[i].b[j].pos.y, anchorZ-c.t[i].b[j].pos.z);
          r2 = new PVector(anchorX + (i+1)*disT - c.t[i].b[j].pos.x, anchorY-c.t[i].b[j].pos.y, anchorZ-c.t[i].b[j].pos.z); 
          va2.add(c.t[i+1].b[j].vel);  
        }
        else{
          r1 = PVector.sub(c.t[i].b[j-1].pos, c.t[i].b[j].pos);
          r2 = PVector.sub(c.t[i+1].b[j-1].pos, c.t[i].b[j].pos);
          va1.add(c.t[i].b[j-1].vel);
          va1.add(c.t[i+1].b[j-1].vel);
          va2.add(c.t[i+1].b[j-1].vel);
          va2.add(c.t[i+1].b[j].vel);
        }
        va1.mult(1.0/3);
        va2.mult(1.0/3);
        r3 = PVector.sub(c.t[i+1].b[j].pos, c.t[i].b[j].pos);
        PVector.cross(r1, r2, n1);
        PVector.cross(r2, r3, n2);
        float tmp = PVector.dot(va1,n1);
        tmp *= (va1.mag()/(2*n1.mag()));
        tmp *= cd/(-2);
        fd1 = PVector.mult(n1, tmp);
        tmp = PVector.dot(va2,n2);
        tmp *= (va2.mag()/(2*n2.mag()));
        tmp *= cd/(-2);
        fd2 = PVector.mult(n2, tmp);
        a1 = PVector.mult(fd1, 1.0/(3*mass));
        a2 = PVector.mult(fd2, 1.0/(3*mass));
        acc_drag[i * numofbead + j].add(a1);
        acc_drag[i * numofbead + j].add(a2);
        if (j > 0) {
          acc_drag[(i * numofbead) + j - 1].add(a1);
          acc_drag[(i+1) * numofbead + j - 1].add(a1);
          acc_drag[(i+1) * numofbead + j - 1].add(a2);
        }
        acc_drag[(i+1) * numofbead + j].add(a2);
      }
    }
    ...
    for (int i = 0; i < numofthread; i++) { // each thread
      for (int j = 0; j < numofbead; j++) { // each bead
        ...
        acc_x += acc_drag[i * numofbead + j].x;
        acc_y += acc_drag[i * numofbead + j].y;
        acc_z += acc_drag[i * numofbead + j].z;  
...
void draw() {
  ...
  if (play) update(0.005); //We're using a fixed, large dt -- this is a bad idea!!
  ...
  //for (int i = 0; i < numofthread; i++) { // each thread
    ...
    /*
      pushMatrix();
      if (j == 0) line(anchorX + i*disT, anchorY, anchorZ, c.t[i].b[j].pos.x, c.t[i].b[j].pos.y, c.t[i].b[j].pos.z);
      else line(c.t[i].b[j-1].pos.x, c.t[i].b[j-1].pos.y, c.t[i].b[j-1].pos.z, c.t[i].b[j].pos.x, c.t[i].b[j].pos.y, c.t[i].b[j].pos.z);
      if (i < numofthread - 1) line(c.t[i].b[j].pos.x, c.t[i].b[j].pos.y, c.t[i].b[j].pos.z, c.t[i+1].b[j].pos.x, c.t[i+1].b[j].pos.y, c.t[i+1].b[j].pos.z);
      translate(c.t[i].b[j].pos.x, c.t[i].b[j].pos.y, c.t[i].b[j].pos.z);
      sphere(radius);
      popMatrix();
      */
      if (i%2 ==0){
        if (j%2 == 0) fill(122, 0, 25);
        else fill(255, 204, 51);
      }
      else{
        if (j%2 != 0) fill(122, 0, 25);
        else fill(255, 204, 51);
      ...
      if (j == 0) {
        vertex(anchorX + i*disT, anchorY, anchorZ);
        vertex(anchorX + (i+1)*disT, anchorY, anchorZ);
      }
      else {
        vertex(c.t[i].b[j-1].pos.x, c.t[i].b[j-1].pos.y, c.t[i].b[j-1].pos.z);
        vertex(c.t[i+1].b[j-1].pos.x, c.t[i+1].b[j-1].pos.y, c.t[i+1].b[j-1].pos.z);
      }
      vertex(c.t[i].b[j].pos.x, c.t[i].b[j].pos.y, c.t[i].b[j].pos.z);
      endShape();
      beginShape(TRIANGLES);
      vertex(c.t[i].b[j].pos.x, c.t[i].b[j].pos.y, c.t[i].b[j].pos.z);
      vertex(c.t[i+1].b[j].pos.x, c.t[i+1].b[j].pos.y, c.t[i+1].b[j].pos.z);
      if (j == 0) vertex(anchorX + (i+1)*disT, anchorY, anchorZ);
      else vertex(c.t[i+1].b[j-1].pos.x, c.t[i+1].b[j-1].pos.y, c.t[i+1].b[j-1].pos.z);
      endShape();
    }
  }
  //endShape();
